      ...
      if (src !== "") {
        ...
        for (let i = 0, iLen = a.length; i < iLen; i++) {
          ...
          if (data2 === null || data2[a[i]] === null) {
            return null;
          } else if (data2 === void 0 || data2[a[i]] === void 0) {
            return void 0;
          }
          data2 = data2[a[i]];
        }
  ...
  if (dataPoint === null) {
    ...
    let setData = function(data2, val, src) {
      ...
      for (let i = 0, iLen = a.length - 1; i < iLen; i++) {
        ...
        if (data2[a[i]] === null || data2[a[i]] === void 0) {
          data2[a[i]] = {};
        }
        data2 = data2[a[i]];
      }
  ...
  add(selector, sort2 = true) {
    if (selector) {
      ...
      selector.length !== void 0) {
        ...
        for (let i = 0; i < arrayLike2.length; i++) {
          addArray(this, arrayLike2[i]);
        }
        sort2 = false;
      } else {
...
var header = (settings, cell, classes2) => {
  ...
  if (
    ...
    titleRow === true && rowIdx !== 0 || titleRow === false && rowIdx !== headerRows.count() - 1 || typeof titleRow === "number" && rowIdx !== titleRow
  ) {
    return;
  }
...
function calculateColumnWidths(settings) {
  ...
  var table = settings.table, columns = settings.columns, scroll = settings.scroll, scrollY = scroll.y, scrollX = scroll.x, visibleColumns2 = getColumns(settings, "visible"), tableWidthAttr = table.getAttribute("width"), tableContainer = table.parentElement, i, j, column, columnIdx;
  ...
  var longestData = [];
  for (i = 0; i < visibleColumns2.length; i++) {
    longestData.push(getWideStrings(settings, visibleColumns2[i]));
  }
  ...
  for (i = 0; i < visibleColumns2.length; i++) {
    ...
    total += bounding;
    columns[visibleColumns2[i]].width = stringToCss(bounding);
  }
...
function visibleColumns(settings) {
  ...
  let vis = 0;
  if (layout2.length) {
    for (let i = 0, iLen = layout2[0].length; i < iLen; i++) {
      if (columns[i].visible && Dom.s(layout2[0][i].cell).css("display") !== "none") {
        vis++;
      }
    }
  }
...
function sortInit(settings) {
  ...
  if (settings.orderHandler) {
    sortAttachListener(settings, target, target === settings.thead ? "tr" + notSelector + " th" + notSelector + ", tr" + notSelector + " td" + notSelector : "th" + notSelector + ", td" + notSelector);
  }
  var order = [];
...
function sort(ctx, col, dir) {
  var i, iLen, aiOrig = [], extSort = ext.type.order, data2 = ctx.data, sortCol, displayMaster = ctx.displayMaster, aSort;
  columnTypes(ctx);
  if (col !== void 0) {
...
function loadState(settings, callback2) {
  ...
  var loaded = function(state2) {
    implementState(settings, state2, callback2);
  };
  var state = settings.stateLoadCallback.call(settings.instance, settings, loaded);
  if (state !== void 0) {
    implementState(settings, state, callback2);
  }
  return true;
}
function implementState(settings, s, callback2) {
  var i, iLen;
  var columns = settings.columns;
  ...
  if (s.order !== void 0) {
    settings.order = [];
    for (let i2 = 0; i2 < s.order.length; i2++) {
      let col2 = s.order[i2];
  ...
  if (s.columns) {
    ...
    if (set2.length === columns.length) {
      for (i = 0, iLen = set2.length; i < iLen; i++) {
        var col = set2[i];
...
function filterCustom(settings) {
  ...
  for (let i = 0, iLen = filters.length; i < iLen; i++) {
    ...
    for (let j = 0, jen = displayRows.length; j < jen; j++) {
      ...
      if (row && filters[i](settings, row.searchCellCache, rowIdx, row.data, j)) {
        rows.push(rowIdx);
      }
    }
  ...
  for (i = 0; i < searchRows.length; i++) {
    ...
    if (row) {
      ...
      if (searchFunc && searchFunc(
        ...
        // compat
      ) || rpSearch && data2 && rpSearch.test(data2)) {
...
function createTr(settings, rowIdx, trIn, tds) {
  var row = settings.data[rowIdx], cells = [], tr, td, column, i, iLen, create2, trClass = settings.classes.tbody.row;
  if (row && row.tr === null) {
    let rowData = row.data;
    ...
    for (i = 0, iLen = settings.columns.length; i < iLen; i++) {
      ...
      create2 = trIn && tds && tds[i] ? false : true;
      td = create2 ? document.createElement(column.cellType) : tds[i];
      if (!td) {
      ...
      cells.push(td);
      var display = getRowDisplay(settings, rowIdx);
      if (create2 || (column.render || column.data !== i) && (!util.is.plainObject(column.data) || column.data && column.data._ !== i + ".display")) {
  ...
  if (side === "header" || util.array.pluck(settings.columns, titleProp).join("")) {
    ...
    if (row.count() === 1) {
      ...
      for (i = cellCount, iLen = columns.length; i < iLen; i++) {
        Dom.c("th").html(columns[i][titleProp] || "").appendTo(row);
      }
    }
  }
  ...
  for (i = 0, len = rows.count(); i < len; i++) {
    ...
    while (loopCell) {
      if (loopCell.nodeName.toUpperCase() == "TD" || loopCell.nodeName.toUpperCase() == "TH") {
        ...
        shifted = shift(layout2, i, column);
...
function toContext(mixedIn) {
  ...
  if (nodes) {
    return settings.filter(function(v, i) {
      return nodes.includes(tables2[i]);
    });
  }
}
...
function columnHeader(settings, column, row) {
  ...
  if (row !== void 0) {
    ...
    for (var i = 0; i < header2.length; i++) {
      if (header2[i][column].unique && Dom.s(header2[i][column].cell).find(".dt-column-title").text()) {
        target = i;
      }
    }
    if (target === null) {
...
function columnHeaderCells(header2) {
  var out = [];
  for (var i = 0; i < header2.length; i++) {
    for (var j = 0; j < header2[i].length; j++) {
      var cell = header2[i][j].cell;
...
function selectColumns(settings, selector, opts) {
  ...
  var run = function(s) {
      ...
      switch (match[2]) {
        ...
        case "visible":
          ...
          return columns.map(function(col, mapIdx) {
            ...
            if (match && match[1]) {
              return Dom.s(nodes[mapIdx]).filter(match[1]).count() > 0 ? mapIdx : null;
            }
            return mapIdx;
          });
        case "name":
    ...
    var result = Dom.s(nodes).filter(s).mapTo((el) => {
...
function setColumnVis(settings, column, vis) {
  var cols = settings.columns, col = cols[column], data2 = settings.data, cells, i, iLen, tr;
  if (vis === void 0) {
    return col.visible;
  }
  if (col.visible === vis) {
...
register(["columns().order()", "column().order()"], function(dir) {
  ...
  if (!dir) {
    return this.iterator("column", function(settings, idx) {
      ...
      for (var i = 0, iLen = sort2.length; i < iLen; i++) {
        if (sort2[i].col === idx) {
          return sort2[i].dir;
        }
      }
...
register("state()", function(set2, ignoreTime) {
  if (!set2) {
...
register$2("pageLength", function(settings, optsIn) {
  ...
  let classes2 = settings.classes.length, tableId = settings.tableId, menu = opts.menu, lengths = [], language = [], i;
  ...
  for (i = 0; i < lengths.length; i++) {
    ...
    if (label === null) {
      label = typeof language[i] === "number" ? settings.formatNumber(language[i], settings) : language[i];
    }
    select2.get(0)[i] = new Option(label, lengths[i]);
  }
